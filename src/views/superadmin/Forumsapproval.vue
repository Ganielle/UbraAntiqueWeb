<template>
    <div>
        ...
        <div v-else>
            ...
            <Forumsapprove :forums="forums" :loading="loading" @approve="UpdateStatusWork" @deny="UpdateStatusWork" @view-details="ViewDescription"/>
        </div>
    </div>
</template>
...
import Forumsapprove from '../../components/UbraAntique/Superadmin/Forums/Forumsapproval.vue';
...
export default {
    ...
    methods: {
        ...
        UpdateStatusWork(id, status){
            if (status == "Deny"){
                this.$swal({
                    title: `Are you sure you want to ${status} this post?`,
                    // html: `
                    //     <center>
                    ...
                    preConfirm: async () => {
                        try {
                            ...
                            const response = await fetch(`${process.env.VUE_APP_API_URL}/forums/updatestatus`,{
                                method: 'POST',
                                headers: {
                                    "Content-Type": "application/json"
                                },
                    ...
                    if (tempresponse.isConfirmed){
                        ...
                        return this.$swal({
                            title: "You have successfully " + status + " this post",
                            icon: "success",
