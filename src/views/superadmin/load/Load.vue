    ...
    <div>
        ...
        <div>
            <div class="mb-3 pt-0" style="display: flex; gap: 10px;">
                <input type="text" placeholder="Search Cashin Request" class="px-2 py-1 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline" v-model="requestsearch" @input="SearchCashinRequest"/>
        ...
        <br/><br/>
        <Loadtable :requestitems="requestlist" @view-receipt="ViewItemImage" @approve-cashin="toggleApproveItem" @denied-cashin="toggleDeniedItem"/>
        <br/><br/>
        <div>
            <div class="mb-3 pt-0" style="display: flex; gap: 10px;">
                <input type="text" placeholder="Search Cashin History" class="px-2 py-1 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline" v-model="historysearch" @input="SearchCashinHistory"/>
        ...
        <br/>
        <div style="display: flex; gap: 10px; justify-content: center; align-items: center;">
            <button class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" @click="PreviousePageHistory" :disabled="historypagination.page <= 0 || historyloading">
                <i class="fas fa-chevron-left"></i>
            </button>

            <p style="font-size: 1.4rem; font-weight: bold;">{{ historypagination.page + 1 }} / {{ historypagination.totalpage }}</p>

            <button class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" @click="NextPageHistory" :disabled="historypagination.page >= historypagination.totalpage || historyloading">
                <i class="fas fa-chevron-right"></i>
            </button>
        </div>
        <br/><br/>
        <Loadtablehistory :requestitems="historylist" @view-receipt="ViewItemImage"/>
    </div>
</template>
...
import Loadtablehistory from '@/components/Subastanauan/Dashboard/Superadmin/load/Loadtablehistory.vue'
...
export default {
    ...
    methods: {
        ...
        async LoadRequestList(){
            ...
            const response = await fetch(`${process.env.VUE_APP_API_URL}/cashin/cashinrequestlist?searchname=${this.requestsearch}&status=Pending&page=${this.requestpagination.page}&limit=10`, {
                method: 'GET',
                headers: {
        ...
        async ApproveDeniedStore(isApproved, requestid){
            if (requestid == ""){
                return;
            }
            ...
            const response = await fetch(`${process.env.VUE_APP_API_URL}/cashin/approvedeniecashinrequest`, {
                method: 'POST',
                headers: {
                "Content-Type": "application/json"
                },
        ...
        async LoadRequestHistory(){
            ...
            const response = await fetch(`${process.env.VUE_APP_API_URL}/cashin/cashinrequesthistory?searchname=${this.historysearch}&page=${this.historypagination.page}&limit=10`, {
                method: 'GET',
                headers: {
                "Content-Type": "application/json"
                },
